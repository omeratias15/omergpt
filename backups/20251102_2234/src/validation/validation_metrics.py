...
# === AUTO PATCH START ===
# Validation KPI gates (Sharpe>1.5, IR>1.0)
try:
    if 'sharpe_ratio' in locals() and sharpe_ratio < 1.5:
        raise ValueError(f"Sharpe ratio {sharpe_ratio} below research threshold 1.5")
    if 'information_ratio' in locals() and information_ratio < 1.0:
        raise ValueError(f"Information ratio {information_ratio} below research threshold 1.0")
except Exception as e:
    print("[AUTO PATCH] KPI Gate:", e)
# === AUTO PATCH END ===
