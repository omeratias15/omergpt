...
# === AUTO PATCH START ===
# Added drift→retrain trigger
try:
    from datetime import datetime
    from src.adaptive_learning import trainer
    if 'drift_detected' in locals() and drift_detected:
        print(f"[AUTO PATCH] Drift exceeded at {datetime.utcnow()} — triggering retrain.")
        trainer.run_retrain_job()
except Exception as e:
    print("[AUTO PATCH] Drift→Retrain trigger failed:", e)
# === AUTO PATCH END ===
