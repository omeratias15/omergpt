...
# ==================== DEMO & CONTINUOUS RUNNER ====================

async def run_detector(mode: str = "demo"):
    """
    ...
    if mode == "demo":
        ...
        # Continuous mode with latency tracking
        latency_log = []
        iteration = 0
        try:
            while True:
                ...
                p95 = (
                    statistics.quantiles(latency_log, n=100)[94]
                    if len(latency_log) >= 10 else elapsed_ms
                )

                logger.info(
                    f"[{iteration:05d}] Batches={stats['batches_processed']} | "
                    f"Anomalies={stats['anomalies_detected']} | "
                    f"Critical={stats['critical_anomalies']} | p95={p95:.1f}ms | GPU={GPU_AVAILABLE}"
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run GPU Isolation Forest detector")
    parser.add_argument("--mode", choices=["demo", "continuous"], default="demo")
