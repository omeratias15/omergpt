...
# === AUTO PATCH START ===
# Auto-added: KPI tiles for Sharpe, IR, and Correlation Decay
try:
    import streamlit as st
    st.subheader("📊 Model KPIs (per research)")
    st.metric("Sharpe Ratio", round(globals().get('sharpe_ratio', 0), 2))
    st.metric("Information Ratio", round(globals().get('information_ratio', 0), 2))
    st.metric("Correlation Decay", round(globals().get('correlation_decay', 0), 2))
except Exception as e:
    print("[AUTO PATCH] KPI tile injection failed:", e)
# === AUTO PATCH END ===
