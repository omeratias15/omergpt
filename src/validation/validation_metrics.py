...
# === AUTO PATCH START ===
# Auto-added: validation gate for Sharpe/IR per research
try:
    if 'sharpe_ratio' in locals() and sharpe_ratio < 1.5:
        raise ValueError("Sharpe ratio below 1.5 — model validation failed.")
    if 'information_ratio' in locals() and information_ratio < 1.0:
        raise ValueError("Information ratio below 1.0 — model validation failed.")
except Exception as e:
    print("[AUTO PATCH] Validation gate triggered:", e)
# === AUTO PATCH END ===
